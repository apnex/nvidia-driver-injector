...
- **Does not provide application-transparent reattach on hot-replug.** v4 makes the UNPLUG half survivable (host stays alive; cleanup completes; new `OBJGPU` can be created via fresh `.probe` when the device reappears) but does NOT clear sink-state on the original `pGpu` nor resurrect it in place. Applications holding device handles to the lost GPU must release and reacquire after re-enumeration. Application-transparent reattach (sink-clear path + in-place re-init + workload migration) is OUT of scope for v4 and is a later-phase design problem. Acceptable workflow under v4: unplug → host stays healthy → replug → Linux re-enumerates → fresh OBJGPU via probe → workload pod restart (e.g., `kubectl rollout restart`) acquires the new GPU.
- **Does not address uvm global fatal 0x60 propagation outside our gating patch.** C6 (F1, Phase 2) closes the cross-contamination via gpumgr-walk gating. Anything deeper in the upstream uvm escalation stack is NVIDIA-owned and out of scope.
