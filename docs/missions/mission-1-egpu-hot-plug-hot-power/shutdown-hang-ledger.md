...
| SH-2 | characterize the ~600 ms poll | **RESOLVED 2026-05-30** via PMU sampling (kprobe was IBT-blocked; `bpftrace profile` is breakpoint-free → works on closed RM). The ~600 ms is a **GSP RM-unload RPC**: `rm_shutdown_adapter → kgspUnloadRm_IMPL → _issueRpcAndWait → _kgspRpcRecvPoll` busy-polls the GSP mailbox/heartbeat until the GSP firmware completes the unload (~600 ms TB4 RPC round-trip). NOT a stuck bit; NOT reducible by us. Init path uses the SAME `_kgspRpcRecvPoll` → key lead for #282. See `experiments/SH-2-eBPF-register-identity.md`. |
