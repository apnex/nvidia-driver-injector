...
- **Wedge SITE — now PINNED for the D0/contained regime (Lane 2 Rung 4, n=4, 2026-05-30).** PMU sampling of the A6-contained fire pins the worker at `nv_open_device → rm_init_adapter → RmInitAdapter → kgspInitRm_IMPL → kgspBootstrap_GH100 → gpuTimeoutCondWait(_kgspLockdownReleasedOrFmcError)` — the **GSP never releases lockdown after FSP boot**. This is *earlier* than the `kgspWaitForRmInitDone → rpcRecvPoll(GSP_INIT_DONE)` claim in §TL;DR item 1: the lockdown-release cond-wait is where the host-side wait sits first; the `GPU_IS_LOST`-at-`GSP_INIT_DONE` the `verify-wedge` journal recorded is the *eventual* outcome when the wait is allowed to run its full (no-A6) multi-second course. Both are the same failing GSP boot, observed at different cut-points. The early sanity-check MMIO (`0x110094`) hypothesis is **falsified** (0 PMU hits). **Rung 5: NO PCIe CTO/AER fires in the contained fire** — the polled lockdown MMIO *returns* ("still locked"); the `UESta=0x4000` the historical no-A6 wedge showed is a *later* chip-drop that A6's 200 ms pre-empts. So the host-lock mechanism is a **multi-second busy-poll holding the GPU group lock**, not a hung-read CTO — the Lane-3 (no-A6) question.
