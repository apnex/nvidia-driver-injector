...
**✅ RESULT — 2026-06-01 (CONTAINED, n=10, 0 fails; KFENCE not KASAN per amendment).** Runner `tools/oa-harness/rung2.sh` (drain-first + no-persistence F40 precond + cycle-1 destructive close + cycle-2 fire). Smoke n=1 then bound n=10 (`r2-adversarial-bound-*`). **All 10 iterations were genuine divergent fires** (`open timed out`, not "completed within budget") and **every one contained**: bounded `-EIO` 205–214 ms, host survived, **every `scheduled` matched by a `timed out`/`completed` (worker JOINED)** — zero unmatched, `tb_egpu_f40b_fires` incremented, **0 KFENCE UAF** at `sample_interval=1`, each re-recovered to BAR1 32768. The wedgeIf never triggered. **R0 stop-rule — partial, SUBSTRATE-LIMITED (corrected 2026-06-01 after adversarial review + forensic verification):** the precond reproduces the **WPR2-already-up fast-fail** divergence (`_kgspBootGspRm: unexpected WPR2 already up` → `RmInitAdapter failed 0x62:0x40:2131`), where the worker reaches the WPR2 check at ~200 ms and fails → flush_work join ≈ 10 ms, total ~210 ms. This does **NOT** exercise the `gpuTimeoutCondWait(_kgspLockdownReleasedOrFmcError)` GSP-lockdown busy-poll that the R0 stop-rule was actually written for. **The worst case is now SOURCE-SETTLED (2026-06-02 workflow `lockdown-poll-substrate-experiment-design` + my own source verification — NO live fire, which would be unreproducible + H-OA6-wedge-risky):** on a true lockdown substrate the open-timeout foreground blocks up to RM `gpuTimeout` (~4 s graphics / ~30 s compute) holding `ldata_lock`, and the block lands at **`nv.c:1905` `rm_cleanup_gpu_lost_state` → `rmapiLockAcquire(API_LOCK_FLAGS_NONE,…)` — a BLOCKING acquire (NONE lacks the `COND_ACQUIRE` non-block bit; the "API lock contended, deferring" branch at osapi.c:1922 is reached on lock-ERROR, not contention), one line BEFORE `flush_work` (`nv.c:1943`)** — because the worker holds that same RM API lock across `RmInitAdapter → kgspBootstrap_GH100 → gpuTimeoutCondWait`. **Implications:** (1) A6's effective bound is `gpuTimeout`, NOT 200 ms, on the lockdown substrate (the WPR2-fast-fail twin is the only ~210 ms case); (2) the C5 sink is INEFFECTIVE there (blocks instead of setting the sink); (3) the deferred-open redesign alone would NOT fix it (block is upstream of `flush_work`); the only kernel-open levers are **reduce `gpuTimeout`** or accept the bounded soft-block floor — the real fast-fail fix is NVIDIA's (#979). **Size E27/A9 in-driver bounded-wait against `gpuTimeout`.** The lockdown substrate is not userspace-reproducible on demand (WPR2-already-up is an H-OA1 sequela); the remaining open item is the deterministic **fake-5090 synthetic** model of "WPR2-clear + GSP boots + lockdown never releases" as the reboot-free CI gate. (n=10 iter-1 also showed a single open fanning out to 16 bounded-worker dispatches, all 16 matched/joined — valid for the lifecycle/leak invariant regardless of substrate.)
