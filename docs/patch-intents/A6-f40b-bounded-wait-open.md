...
Close the F40-open-arm host-wedge class on userspace-recovered Blackwell eGPUs over Thunderbolt 4. Chip-touching MMIO inside `RmInitAdapter` (reached from `nv_open_device_for_nvlfp`) can hang indefinitely on a chip that has been recovered in userspace via the `rmmod + TB cycle + fix-bar1 + modprobe` sequence — the chip does not produce a PCIe completion, the syscall thread deadlocks holding the global lock, and the kernel wedges before AER processing can fire the C5 sink. A6 schedules the chip-touching open onto `system_long_wq`, lets the syscall thread wait with a bounded timeout (default 200 ms = 4× PCIe Completion Timeout), and on timeout declares the GPU lost via the C5 sink primitive and returns `-EIO`. Userspace sees the same error code it would see on a successful kernel AER race; the wrapper makes the outcome deterministic instead of timing-dependent. See `fake-5090/failure-modes/F40-reinit-gsp-lockdown-wedge.md` and `docs/missions/mission-1-egpu-hot-plug-hot-power/design/F40b-structural-fix-2026-05-29.md` for mechanism evidence (n=13 wedges + Test B v2 + TBv2-n2 + F40B-TEST n=2 validation).

**Healthy-path note (added 2026-05-29 evening from A7 Test A observations):** A6's wrapper fires on EVERY open of /dev/nvidia0 on an E1-classified eGPU with the feature enabled, not just on F40-precondition opens. On healthy chips, the worker completes well within the 200 ms budget — `nvidia-smi -pm 1` at persistence engagement triggers 2-3 wrapped opens per pod start, all logging "completed within budget rc=0" within ms of the schedule line. A6's overhead on healthy opens is negligible (single worker hop, kernel completion); A6's value is the timeout branch on the rare wedge path. The "fires on every open" property is by design — there is no way for the wrapper to know in advance whether THIS open will be the wedged one.

> **⚠️ CORRECTION 2026-06-04 — the "well within 200 ms" claim is true only for WARM opens, NOT a healthy full cold init.** The 2026-05-29 observation was on opens where the adapter was *already initialised* (`nvidia-smi -pm 1` runs the cold init via the un-A6-bounded H-OA2 path, so the A6-bounded opens it triggers are warm attaches of µs). A healthy **full cold init** (`RmInitAdapter`/GSP bootstrap) through the A6-bounded H-OA1 path measures **~1.3 s** (n=3, 2026-06-04 `fastfail` run; KFENCE-controlled via the shutdown op, so not an artifact) — **6× the 200 ms budget**. With `GpuInitOnProbe=0` (lazy init), a client opening `/dev/nvidia0` while the adapter is uninitialised (e.g. a CUDA consumer before persistence engages, or racing a hot-replug recovery, or any non-persistence deployment) takes H-OA1, trips A6 at 200 ms, exhausts the 50 ms A10 grace mid-init, and the A10-v2 lockdown arm **dead-buses a healthy chip**. The 200 ms budget (= "4× PCIe Completion Timeout", a wedge-detection threshold) was never tuned against a full cold init — the exact twin of the A7 shutdown-budget bug (#279). Fix = raise budget (+ grace) above the worst-case healthy cold init so it completes-within-budget / fast-fails. Full analysis + proposed change: `docs/missions/mission-1-egpu-hot-plug-hot-power/finding-2026-06-04-a6-open-budget-vs-healthy-cold-init.md`.
...
Userspace consumers SHOULD monitor for the "timed out" line via `journalctl -k -f` to detect F40b fires. A8 publishes the same fact via the `tb_egpu_f40b_fires` sysfs counter (NOTE: A8 v1 sysfs surface failed to materialise at runtime despite symbols loading — A8 v2 forthcoming will switch to the explicit `sysfs_create_group` pattern A3 uses. Until A8 v2 ships, journalctl is the only reliable A6/A7 telemetry surface).

## Provenance

- **Source cluster**: addon — project-local; F40-open-arm wedge containment (companion to A7 which closes the F40-shutdown-arm). The F40 chip-side root cause is NVIDIA bug #979 (`project_issue_979_upstream_state_2026_05_22.md`) and is out of scope for our fork; A6 is the kernel-side containment for the open path.
- **Empirical validation**:
  - **F40B-TEST n=2 (2026-05-29 evening, 19:48 and 19:50):** wrapper TIMEOUT branch validated. Cycle-2 open of a userspace-recovered chip via `exec 3</dev/nvidia0` triggered the F40 wedge precondition; wrapper timed out at 201.7 ms (run 1) and 203.5 ms (run 2); -EIO returned to bash; host alive.
  - **A7 Test A n=2 (2026-05-29 evening, 21:56:55 and 22:06:35):** wrapper HEALTHY-PATH branch observed in passing. Persistence engagement via `nvidia-smi -pm 1` triggered 2-3 wrapped opens per run; all completed within budget rc=0 within ms. See `docs/missions/mission-1-egpu-hot-plug-hot-power/design/A7-test-A-validation-2026-05-29.md` for the kernel-log signatures.
  - **R0 leak→join (2026-05-31, apnex.25):** the timeout branch was source-confirmed (R0 correctness audit, 4-agent) to LEAK the worker, which writes `nvlfp`/`sp` after `nvidia_open`'s `failed:` path frees them — a use-after-free (`fake-5090` F42), *widened* by A9 (every first-open now dispatches a leakable worker). R0 adds `flush_work(&w->work)` (mirroring A7's SH-3 guard) to JOIN the worker before return → UAF closed, **deadlock-free** (disjoint lock domains — flushing syscall under `ldata_lock` vs worker under the RM group lock; audit-verified), compile-validated. CAVEAT recorded in-code + the v5 queue: the C5 sink does NOT fast-fail this poll (blocking RM API-lock acquire), so the timeout branch blocks up to the RM `gpuTimeout` (~4 s/~30 s) holding `ldata_lock` — the structural fix (route the bounded open through the existing deferred-open lifecycle) is queued for v5; R0 is the safe interim floor.
- **Vanilla baseline files**: `kernel-open/nvidia/nv.c` (insertion of `nv_open_device_for_nvlfp_bounded` and supporting `struct nv_f40b_open_work` / worker / put primitives between `nv_open_device_for_nvlfp` and `nvidia_open_deferred`; one-line call-site change in `nvidia_open`'s foreground path; R0 added `flush_work(&w->work)` in the timeout branch).
- **Fork branch**: `a6-f40b-bounded-wait-open` (in `/root/open-gpu-kernel-modules`; not yet pushed to apnex fork).
- **Injector main commits**: `c8d3c68` (A6 patch first landed without intent doc) → `429615c` (A6 catch-up intent doc + A7 patch + A8 patch + renumber).
- **Image first deployed**: `apnex/nvidia-driver-injector:595.71.05-aorus.18-f40b` (A6 alone, 2026-05-29 afternoon) → superseded by `595.71.05-aorus.19` (A6 + A7 + A8, 2026-05-29 evening).
