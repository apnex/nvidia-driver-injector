...
- **Image first deployed**: `apnex/nvidia-driver-injector:595.71.05-apnex.24` (A9 + A8 v2.2), deployed 2026-05-31. **Healthy-deploy VERIFIED non-destructively:** E1's `external GPU detected` fires at PROBE (before the first open; apnex.23 fired it 6 s later during the first open's RmInitAdapter), the first open is A6-wrapped, and `tb_egpu_is_external` reads 1. The destructive first-open-on-bad-chip test (survivability) remains the gated Phase-3 follow-up.
