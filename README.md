...
- `--revert-cmdline` — strip the kernel cmdline args this repo added. Reboot required.
- `--purge` — deep clean for fresh-host testing (implies `--revert-cmdline`, removes the on-disk `.ko`, restores legacy ICDs). Reboot required.
...
Run `sudo ./scripts/apply.sh` and reboot.
...
The upstream image is on `debian:13-slim` (gcc 14.2) which covers kernels built with gcc 13/14/15.
